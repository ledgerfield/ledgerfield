...
import json, hashlib
from ledgerfield.schemas.MX.schema import MX_ACCOUNTS, get_account


def test_schema_has_accounts():
    assert len(MX_ACCOUNTS) >= 20
...
def test_ruleset_json_valid():
    rs = json.load(open('/tmp/ledgerfield/rulesets/MX_2025.json'))
    assert rs["jurisdiction"] == "MX"
