...
import json, hashlib
from ledgerfield.schemas.FI.schema import FI_ACCOUNTS, get_account

def test_schema_has_accounts():
    assert len(FI_ACCOUNTS) >= 20
...
def test_ruleset_json_valid():
    rs = json.load(open('/tmp/ledgerfield/rulesets/FI_2025.json'))
    assert rs["jurisdiction"] == "FI"
