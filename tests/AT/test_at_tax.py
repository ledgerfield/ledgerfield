...
import json, hashlib
from ledgerfield.schemas.AT.schema import AT_ACCOUNTS, get_account

def test_schema_has_accounts():
    assert len(AT_ACCOUNTS) >= 20
...
def test_ruleset_json_valid():
    rs = json.load(open('/tmp/ledgerfield/rulesets/AT_2025.json'))
    assert rs["jurisdiction"] == "AT"
