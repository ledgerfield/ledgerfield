...
import json, hashlib
from ledgerfield.schemas.BR.schema import BR_ACCOUNTS, get_account

def test_schema_has_accounts():
    assert len(BR_ACCOUNTS) >= 20
...
def test_ruleset_json_valid():
    rs = json.load(open('/tmp/ledgerfield/rulesets/BR_2025.json'))
    assert rs["jurisdiction"] == "BR"
