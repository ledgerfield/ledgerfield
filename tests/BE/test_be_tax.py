...
import json, hashlib
from ledgerfield.schemas.BE.schema import BE_ACCOUNTS, get_account

def test_schema_has_accounts():
    assert len(BE_ACCOUNTS) >= 20
...
def test_ruleset_json_valid():
    rs = json.load(open('/tmp/ledgerfield/rulesets/BE_2025.json'))
    assert rs["jurisdiction"] == "BE"
