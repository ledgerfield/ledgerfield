...
import json, hashlib
from ledgerfield.schemas.HK.schema import HK_ACCOUNTS, get_account

def test_schema_has_accounts():
    assert len(HK_ACCOUNTS) >= 20
...
def test_ruleset_json_valid():
    rs = json.load(open('/tmp/ledgerfield/rulesets/HK_2025.json'))
    assert rs["jurisdiction"] == "HK"
