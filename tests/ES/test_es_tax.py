...
import json, hashlib
from ledgerfield.schemas.ES.schema import ES_ACCOUNTS, get_account

def test_schema_has_accounts():
    assert len(ES_ACCOUNTS) >= 20
...
def test_ruleset_json_valid():
    rs = json.load(open('/tmp/ledgerfield/rulesets/ES_2025.json'))
    assert rs["jurisdiction"] == "ES"
