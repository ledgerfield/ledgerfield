...
import json, hashlib
from ledgerfield.schemas.CH.schema import CH_ACCOUNTS, get_account

def test_schema_has_accounts():
    assert len(CH_ACCOUNTS) >= 20
...
def test_ruleset_json_valid():
    rs = json.load(open('/tmp/ledgerfield/rulesets/CH_2025.json'))
    assert rs["jurisdiction"] == "CH"
